...
class GPT3ComCon(discord.Cog, name="GPT3ComCon"):
    ...
    async def end_conversation(
        ...
        if isinstance(
            ...
            await ctx.respond(
                "You have ended the conversation. Start a conversation with /gpt converse",
                ephemeral=True,
                delete_after=10,
            )
        elif isinstance(
    ...
    async def check_conversation_limit(self, message):
        ...
        if message.channel.id in self.conversation_threads:
            ...
            if (
                ...
                await message.reply(
                    "You have reached the maximum conversation length. You have ended the conversation, and it has ended."
                )
    ...
    async def help_command(self, ctx):
        ...
        embed.add_field(
            name="/gpt ask",
            value="Ask something. Be clear, long, and concise in your prompt. Don't waste tokens.",
            inline=False,
        )
        embed.add_field(
            name="/gpt edit",
        ...
        embed.add_field(
            name="/gpt converse", value="Start a conversation", inline=False
        )
        embed.add_field(
            name="/gpt end",
            value="End a conversation. You can also type `end` in the conversation.",
            inline=False,
        )
        embed.add_field(
    ...
    async def converse_command(
        ...
        if private:
            await ctx.respond(
                embed=discord.Embed(
                    title=f"{user.name}'s private conversation",
                    color=0x808080,
                )
            )
            thread = await ctx.channel.create_thread(
                name=user.name + "'s private conversation",
                auto_archive_duration=60,
            )
        elif not private:
            message_thread = await ctx.respond(
                embed=discord.Embed(
                    title=f"{user.name}'s conversation", color=0x808080
                )
            )
